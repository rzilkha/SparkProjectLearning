...
/*
"aws s3 cp s3://roeezilkha/movies.dat ./"
"aws s3 cp s3://roeezilkha/ratings.dat ./"
"aws s3 cp s3://roeezilkha/MovieSimilaritiesForEMR.scala ./"

spark-submit --executor-memory 1g --class example.MovieSimilarities  SparkProject-1.0-SNAPSHOT.jar
*/

/**
  * Created by Roee Zilkha on 11/4/2017.
  */
object MovieSimilaritiesForEMR {


  def loadMovieNames(): Map[Int, String] = {
    var lookup = Map[Int, String]()
    val lines = Source.fromFile("movies.dat", "iso-8859-1").getLines
    for (line <- lines) {
      val arr = line.split("\\|")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rdd = sc.textFile("s3n://roeezilkdsasddasha/ratings.dat").map(parse)

    // (m1,m2): (r1,r2),(r3,r4) ...(rn,rm)
    val moviePairsRatings = rdd.partitionBy(new HashPartitioner(100)).join(rdd).
